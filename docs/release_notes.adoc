...
* Kudu integrators can now write Java tests that communicate with a Kudu server
  without having to first build or install Kudu. This is done by providing
  various platform-specific binaries available to Gradle or Maven for download
  and install at test time. The binary test artifact is still an experimental
  feature.
