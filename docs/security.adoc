...
user to decide whether to perform or reject an request.
...
commonly "impala". Since authorizes requests on its own, if using Impala, to
avoid extraneous communication between Sentry and Kudu, the Impala service user
