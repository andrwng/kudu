...
will write its data blocks. This is a comma-separated list of diectories; if
...
specified, data blocks will be placed in directory specified by `--fs_wal_dir`.
Additionally, `--fs_wal_dir` may be the same as _one of_ the directories listed
in `--fs_data_dirs`, but must not be a sub-directory of any of them.
...
|--fs_data_dirs | string | | List of directories where the Master will place
its data blocks.
|--fs_wal_dir | string | | The directory where the Master will
place its write-ahead logs.
