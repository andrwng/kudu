...
const MonoDelta kAgreementTimeout = MonoDelta::FromSeconds(120);
...
class DiskFailureITest : public TabletServerIntegrationTestBase,
 ...
 public:
  ...
  string GlobForBlockFileInDataDir(const string& data_dir) const {
    if (GetParam() == "file") {
  ...
  vector<string> GetClusterFlags() const {
    vector<string> flags = {
  ...
  void SetupDefaultCluster(vector<ExternalTabletServer*>* ext_tservers) {
    FLAGS_num_tablet_servers = 3;
    vector<string> flags = GetClusterFlags();
    NO_FATALS(CreateCluster("survivable_cluster", flags, {}, /* num_data_dirs */ 2));
  ...
  void SetServerSurvivalFlags(const vector<ExternalTabletServer*>& ext_tservers) {
  ...
  void WaitForDiskFailures(const ExternalTabletServer* ext_tserver,
    ...
    ASSERT_EVENTUALLY([&] {
      ...
      ASSERT_OK(ext_tserver->GetInt64Metric(&METRIC_ENTITY_server, nullptr, &METRIC_data_dirs_failed,
          "value", &failed_on_ts));
...
TEST_P(DiskFailureITest, TestFailDuringFlushMRS) {
  ...
  NO_FATALS(GetDataDirsWrittenToByWorkload(ext_tservers, &workload_a, 3, &dirs_with_A_data,
                                           &dirs_without_A_data));
  NO_FATALS(WaitForTSAndReplicas(kTableIdA));
  string tablet_id_a = (*tablet_replicas_.begin()).first;
  ASSERT_OK(WaitForServersToAgree(kAgreementTimeout, tablet_servers_, tablet_id_a, 0));
  ...
  ASSERT_OK(WaitForServersToAgree(kAgreementTimeout, tablet_servers_, tablet_id_b, 0));
  ...
  NO_FATALS(WaitForDiskFailures(ext_tservers[1]));
...
TEST_P(DiskFailureITest, TestFailDuringFlushDMS) {
  ...
  NO_FATALS(GetDataDirsWrittenToByFunction(ext_tservers, f, 3, &paths_with_data));
  NO_FATALS(WaitForTSAndReplicas(kTableId));
  ...
  for (int ts = 0; ts < ext_tservers.size(); ts++) {
    ...
    NO_FATALS(UpsertPayload(kTableId, 0, 100));
    NO_FATALS(WaitForDiskFailures(ext_tservers[ts], 1));
    ASSERT_OK(WaitForServersToAgree(kAgreementTimeout, tablet_servers_, tablet_id, 0));
  }
}
...
TEST_P(DiskFailureITest, TestFailDuringScan) {
  ...
  // behavior, it doesn't matter how it's inserted.
  ...
  NO_FATALS(WaitForTSAndReplicas(TestWorkload::kDefaultTableName));
  string tablet_id = (*tablet_replicas_.begin()).first;
  ASSERT_OK(WaitForServersToAgree(kAgreementTimeout, tablet_servers_, tablet_id, 0));
...
#else
INSTANTIATE_TEST_CASE_P(DiskFailure, DiskFailureITest, ::testing::Values("file"));
#endif
