...
class MiniClusterFsInspector {
 public:
  virtual ~MiniClusterFsInspector() {}

  // Returns an env with which to operate on the cluster.
  virtual Env* env() const = 0;

  // Returns the number of tablet servers in the cluster.
  virtual int num_tablet_servers() const = 0;

  // Returns the UUID of the TS 'index'.
  virtual std::string UUIDforTS(int index) const = 0;

  // Returns the WALs root directory created for TS 'index'.
  virtual std::string WalRootForTS(int idx) const = 0;

  // Returns the WALs FS subdirectory created for TS 'index'.
  std::string WalDirForTS(int idx) const;
