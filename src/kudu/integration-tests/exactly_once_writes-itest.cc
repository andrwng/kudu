...
TEST_F(ExactlyOnceSemanticsITest, TestWritesWithExactlyOnceSemanticsWithChurnyElections) {
  ...
  // any progress at all.
  raft_heartbeat_interval = 5;
#else
  raft_heartbeat_interval = 2;
#endif
  // Inject random latency of up to twice the Raft heartbeat interval to ensure
  // there will definitely be missed heartbeats, triggering actual elections.
  ts_flags = {
    Substitute("--raft_heartbeat_interval_ms=$0", raft_heartbeat_interval),
    Substitute("--consensus_inject_latency_ms_in_notifications=$0", raft_heartbeat_interval * 2),
