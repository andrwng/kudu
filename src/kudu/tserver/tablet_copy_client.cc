...
Status TabletCopyClient::Abort() {
  ...
  // in a tombstoned state.
  SCOPED_CLEANUP({
    // XXX(awong): This check is guaranteed because we call DeleteTabletData(),
    // although depending on that makes me thing we should split it into
    // SetMetadataState() and FlushMetadata() calls or something.
