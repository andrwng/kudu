...
using kudu::tserver::CoordinateTransactionResponsePB;
using std::string;
...
RetriableRpcStatus CoordinatorRpc::AnalyzeResponse(const Status& rpc_cb_status) {
  ...
  // If we didn't fail on tablet lookup/proxy initialization, check if we
  // failed actually performing the operation.
  if (rpc_cb_status.ok()) {
