...
class BlockManagerStressTest : public KuduTest {
 public:
  BlockManagerStressTest() :
    ...
    bm_->Open(nullptr);
    bm_->dd_manager()->CreateDataDirGroup(test_tablet_name_);
  ...
  BlockManager* CreateBlockManager() {
    ...
    dd_manager_.reset(new DataDirManager(env_, scoped_refptr<MetricEntity>(), T::BlockManagerType(),
        data_dirs, DataDirManager::AccessMode::READ_WRITE));
    dd_manager_->Init();
    dd_manager_->Create();
    dd_manager_->Open();
