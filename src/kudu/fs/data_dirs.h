...
class DataDirManager {
 ...
 private:
  ...
  // Writes the instances in 'dirty_idx_list' to reflect the failed dirs as
  // seen by 'path_set_'. If any instance fails in the process, 'path_set_'
  ...
  // state out of its jurisdiction (e.g. tablets) needs to be updated.
