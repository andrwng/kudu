...
class PathInstanceMetadataFile {
 public:
  ...
  // file, or, in the case of disk failure, 'healthy_instance_' is set to false.
  Status LoadFromDisk();
  ...
  bool healthy_instance() const {
    return healthy_instance_;
  }
  ...
  // 'updated_indices' is populated with indices within 'instances' of those
  // that need to be rewritten to sync with 'main_pb'.
