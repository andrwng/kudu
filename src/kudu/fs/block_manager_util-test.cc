...
TEST_F(KuduTest, CheckIntegrity) {
  ...
  }
  {
    ...
    vector<PathSetPB> path_sets_copy(path_sets);
    for (PathSetPB& ps : path_sets_copy) {
      ps.mutable_deprecated_all_uuids()->Reserve(kAllUuids.size());
      ps.mutable_deprecated_all_uuids()->CopyFrom(kAllUuids);
      ps.clear_all_paths();
  ...
  }
  {
    ...
    for (int i = 0; i < last_idx; i++) {
      path_sets_copy[i].mutable_deprecated_all_uuids()->Reserve(kAllUuids.size());
      path_sets_copy[i].mutable_deprecated_all_uuids()->CopyFrom(kAllUuids);
