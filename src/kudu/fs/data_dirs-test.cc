...
TEST_F(DataDirGroupTest, TestFailedDirNotReturned) {
  ...
  ASSERT_STR_CONTAINS(s.ToString(), "No healthy directories exist in tablet's DataDirGroup");
}
...
TEST_F(DataDirGroupTest, TestFailedDirNotAddedToGroup) {
  ...
  // Check that all uuid_indices are valid and are not the in the failed
  // directory (uuid_idx 0).
