...
void DataDir::SetFailed() {
  is_failed_ = true;
}

bool DataDir::is_failed() const {
  return is_failed_ || metadata_file_->healthy();
}

void DataDir::ExecClosure(const Closure& task) {
  CHECK(instance()->healthy()) << "Executing task for an unhealthy directory";
...
Status DataDirManager::Open(int max_data_dirs, LockMode mode) {
  ...
  for (const auto& dd : dds) {
    ...
    // TODO(awong)
    // Move this after we set the maps.
