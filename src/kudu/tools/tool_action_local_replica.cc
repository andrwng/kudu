...
Status DeleteLocalReplica(const RunnerContext& context) {
  ...
  RETURN_NOT_OK(TabletMetadata::Load(&tmeta_manager, tablet_id, &meta));
...
Status SummarizeDataSize(const RunnerContext& context) {
  ...
  for (const string& tablet_id : tablets) {
    ...
    scoped_refptr<TabletMetadata> meta;
    RETURN_NOT_OK_PREPEND(TabletMetadata::Load(&tmeta_manager, tablet_id, &meta),
...
Status DumpMeta(const RunnerContext& context) {
  ...
  TabletMetadataManager tmeta_manager(fs_manager.get());
  const string& tablet_id = FindOrDie(context.required_args, kTabletIdArg);
  return DumpTabletMeta(fs_manager.get(), &tmeta_manager, tablet_id, 0);
}
...
Status DumpDataDirs(const RunnerContext& context) {
  ...
  RETURN_NOT_OK(TabletMetadata::Load(&tmeta_manager, tablet_id, &unused_meta));
