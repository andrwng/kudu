...
TServerState PickRandomState() {
  switch (rand() % 2) {
    case 0:
      return kNone;
    case 1:
      return kMaintenanceMode;
  }
  return kNone;
}
...
class TServerStateTest : public KuduTest {
 public:
  ...
  Status SetTServerState(const string& tserver_uuid, TServerState state) {
    return master_->catalog_manager()->SetTServerState(tserver_uuid, state);
  }

  // Sends a heartbeat to the master from the given tserver.
...
TEST_F(TServerStateTest, TestReloadTServerState) {
  ...
  ASSERT_OK(SetTServerState(kTServer, kMaintenanceMode));
  ASSERT_EQ(kMaintenanceMode, ts_manager_->GetTServerState(kTServer));
  ...
  ASSERT_OK(SetTServerState(kTServer, kNone));
  ASSERT_EQ(kNone, ts_manager_->GetTServerState(kTServer));
  NO_FATALS(ResetMaster());
  ASSERT_EQ(kNone, ts_manager_->GetTServerState(kTServer));
}
...
TEST_F(TServerStateTest, TestConcurrentSetTServerState) {
  ...
  for (int i = 0; i < kNumTServers; i++) {
    tservers.emplace_back(Substitute("$0-$1", kTServer, i));
  }
  ...
  // state.
  vector<TServerState> in_memory_states;
  for (int i = 0; i < tservers.size(); i++) {
    in_memory_states.emplace_back(ts_manager_->GetTServerState(tservers[i]));
  }
  NO_FATALS(ResetMaster());
  for (int i = 0; i < tservers.size(); i++) {
