...
// A delta iterator that supports merging delta stores that may overlap with
// one another.
template <DeltaType Type>
class OrderedDeltaIteratorMerger :
 ...
 private:
  ...
  OrderedDeltaIteratorMerger(std::vector<std::unique_ptr<DeltaStoreIterator>> store_iters,
                             const RowIteratorOptions& opts);

  DeltaStoreIteratorMerger<Type> merged_store_iter_;
