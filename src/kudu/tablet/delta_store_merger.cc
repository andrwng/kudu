...
template<DeltaType Type>
bool DeltaStoreIteratorMerger<Type>::HasNext() const {
  return !key_heap_.empty() && key_heap_.top().second->HasNext();
}

template<DeltaType Type>
bool DeltaStoreIteratorMerger<Type>::HasMoreBatches() const {
  for (const auto& iter : iters_) {
