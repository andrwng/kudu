...
#undef RETURN_NOT_OK_REPLAY
  ...
  // operations: those whose timestamps are assigned serially with respect to
  // one another.
  bool timestamp_assigned_in_opid_order;
  switch (op_type) {
    case CHANGE_CONFIG_OP:
  ...
  //
  // If this message is a Raft election no-op, or is a write that has an
  // external consistency mode other than COMMIT_WAIT, we know that no future
  // transaction will have a timestamp that is lower than it, so we can just
  // advance the safe timestamp to the message's timestamp.
  //
  // If the hybrid clock is disabled, all transactions will fall into this
  // category.
  Timestamp safe_time;
  if (replicate->op_type() == consensus::NO_OP ||
