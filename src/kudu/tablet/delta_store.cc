...
Status DeltaPreparingIterator<DeltaPreparerType, DeltaStoreIterType>::Init(ScanSpec* spec) {
  initted_ = true;
  return store_iter()->Init(spec);
}

template <class DeltaPreparerType, class DeltaStoreIterType>
Status DeltaPreparingIterator<DeltaPreparerType, DeltaStoreIterType>::SeekToOrdinal(
    rowid_t row_idx) {
...
Status DeltaPreparingIterator<DeltaPreparerType, DeltaStoreIterType>::PrepareBatch(
  ...
  while (store_iter()->HasNext()) {
    ...
    // XXX(awong): this wasn't in DMSIterator but it was in DeltaFileIterator.
...
template <class DeltaPreparerType, class DeltaStoreIterType>
Status DeltaPreparingIterator<DeltaPreparerType, DeltaStoreIterType>::ApplyUpdates(
    size_t col_to_apply, ColumnBlock* dst, const SelectionVector& filter) {
  return preparer()->ApplyUpdates(col_to_apply, dst, filter);
}

template <class DeltaPreparerType, class DeltaStoreIterType>
Status DeltaPreparingIterator<DeltaPreparerType, DeltaStoreIterType>::ApplyDeletes(
    SelectionVector* sel_vec) {
  return preparer()->ApplyDeletes(sel_vec);
}

template <class DeltaPreparerType, class DeltaStoreIterType>
Status DeltaPreparingIterator<DeltaPreparerType, DeltaStoreIterType>::SelectDeltas(
    SelectedDeltas* deltas) {
  return preparer()->SelectDeltas(deltas);
}

template <class DeltaPreparerType, class DeltaStoreIterType>
Status DeltaPreparingIterator<DeltaPreparerType, DeltaStoreIterType>::CollectMutations(
    vector<Mutation*>* dst, Arena* arena) {
  return preparer()->CollectMutations(dst, arena);
}

template <class DeltaPreparerType, class DeltaStoreIterType>
Status DeltaPreparingIterator<DeltaPreparerType, DeltaStoreIterType>::
    FilterColumnIdsAndCollectDeltas(const vector<ColumnId>& col_ids,
                                    vector<DeltaKeyAndUpdate>* out,
                                    Arena* arena) {
...
template <class DeltaPreparerType, class DeltaStoreIterType>
bool DeltaPreparingIterator<DeltaPreparerType, DeltaStoreIterType>::HasNext() const {
  return store_iter()->HasMoreBatches();
}

template <class DeltaPreparerType, class DeltaStoreIterType>
bool DeltaPreparingIterator<DeltaPreparerType, DeltaStoreIterType>::MayHaveDeltas() const {
  return preparer()->MayHaveDeltas();
}
