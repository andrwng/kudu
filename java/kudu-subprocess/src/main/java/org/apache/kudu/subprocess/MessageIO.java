...
/**
 * Util class for read and write protobuf message.
 */
...
class MessageIO {
  ...
  /**
   * Reads <code>size</code> bytes of data from the underlying buffered
   * input stream into the specified byte array, starting at the offset
   * <code>0</code>. If fail to read the specified size, IOException is
   * thrown.
   *
  ...
  private void doRead(byte bytes[], int size) throws EOFException, IOException {
    ...
    if (read == -1) {
      ...
      throw new IOException(
          String.format("unable to receive message, expect (%d) bytes " +
                        "but read (%d) bytes", size, read));
  }

  /**
   * Write a protobuf message to the underlying buffered output stream.
   * The write is atomic, that is if any exceptions occur, no partial
  ...
  /**
   * Convert a four bytes array in big endian order to a 32-bit integer.
   * @param data a four bytes array in big endian order
