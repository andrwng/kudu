...
class MessageReader implements Runnable {
  ...
  @VisibleForTesting
  static final String READ_MSG_ERR = "Unable to read the protobuf message";
  @VisibleForTesting
  static final String PUT_MSG_ERR = "Unable to put the message to the queue";
  ...
  public void run() {
    ...
    while (true) {
      ...
      // Retry on IOException is not necessary as the error can happen
      // in the middle of message reading.
      byte[] data;
      try {
        data = messageIO.readBytes();
      } catch (EOFException e) {
        LOG.info("Reaching the end of the input stream, exit.");
        // Break the loop if the end of the stream has been reached.
        break;
      } catch (IOException e) {
        throw new KuduSubprocessException(READ_MSG_ERR, e);
      }

      // Put the message, if not empty, to the queue. If encountered
      // InterruptedException during the put, consider it to be fatal
      // (as a signal to shutdown the task), and propagate it the code
      ...
      try {
        ...
        throw new KuduSubprocessException(PUT_MSG_ERR, e);
      }
    }
  }
}
